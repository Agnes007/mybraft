...
public:
    ...
    int start() {
        ...
        std::string dbpath = "tdb" + std::to_string(FLAGS_port);
        LOG(INFO) << "leveldb dbpath:"<<dbpath;
        leveldb::Status status = leveldb::DB::Open(options, dbpath, &_ldb);
        assert(status.ok());
        LOG(INFO) << "open leveldb instance ok.";
    ...
    void on_apply(braft::Iterator& iter) {
        ...
        for (; iter.valid(); iter.next()) {
            ...
            if (iter.done()) {
                ...
                //hyf test 10.30 2018
                //此时log已经应用到副本集汇总的所有node的statemachine中，解析传入的key-value-tyoe,调用具体的LevelDB接口
                LOG(INFO) <<"[iter.done=true]key:"<< c->request()->key() << " "<< "val:" << c->request()->val()
                    <<" "<<"type:"<< c->request()->type();
                leveldb::Status ss=_ldb->Put(leveldb::WriteOptions(),c->request()->key(),c->request()->val());
                assert(ss.ok());
                LOG(INFO) << "[iter.done=true]write leveldb ok";
                //*******************************8
            } else {
                ...
                //已经应用到副本集中的所有node的StateMashine中
                LOG(INFO) <<"[iter.done=false]key:"<< request.key() << " "<< "val:" << request.val()
                    <<" "<<"type:"<<request.type();
                leveldb::Status ss=_ldb->Put(leveldb::WriteOptions(),request.key(),request.val());
                assert(ss.ok());
                LOG(INFO) << "[iter.done=true]write leveldb ok";
            }
